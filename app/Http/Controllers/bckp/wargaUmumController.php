...
class wargaUmumController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $scanKtpPath  = str_replace('public/', '', Storage::putFileAs('public/scan_ktp', $request->file('ktp'), 'scan_ktp_umum_' . $user->id . '.jpg'));
            $scanKkPath   = str_replace('public/', '', Storage::putFileAs('public/scan_kk', $request->file('kk'), 'scan_kk_umum_' . $user->id . '.jpg'));
            $scanAktaPath = str_replace('public/', '', Storage::putFileAs('public/scan_akta', $request->file('akta-kelahiran'), 'scan_akta_umum_' . $user->id . '.jpg'));
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            $scanFiles['ktp']  = $request->file('ktp')?->storeAs('public/scan_ktp', 'scan_ktp_umum_' . $user->id . '.jpg');
            $scanFiles['kk']   = $request->file('kk')?->storeAs('public/scan_kk', 'scan_kk_umum_' . $user->id . '.jpg');
            $scanFiles['akta'] = $request->file('akta-kelahiran')?->storeAs('public/scan_akta', 'scan_akta_umum_' . $user->id . '.jpg');
